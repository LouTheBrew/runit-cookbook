...
namespace :repository do
  ...
  task :git_commit_and_push do
    ...
    git tag -a #{} -m "version release #{readit 'VERSION'}"
...
driver:
  ...
  tags:
    ...
    Cookbook: #{readit 'NAME'}
    Cookbook_Version: #{readit 'VERSION'}
    Developer: #{ENV['USER']}
...
namespace :notifications do
  task :status do
    puts "Cookbook Name: #{readit 'NAME'}"
    puts "Cookbook Version: #{readit 'VERSION'}"
...
namespace :special do
  ...
  task :wipe do
    resource = Aws::EC2::Resource.new
    destroyed = 0
    resource.instances({:filters => [{name: 'tag:Cookbook', values: [readit('NAME')]}]}).each do |instance|
      puts instance.id
      response = instance.terminate(dry_run: false)
      if response.successful?
        destroyed += 1
    ...
    if destroyed > 0
      puts "Ran a check for extra instances related to cookbook #{readit 'NAME'} and #{destroyed} were sent a terminate signal, these may have been previously terminated"
    end
  end
end
